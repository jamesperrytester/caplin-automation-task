...
//             url: /indices\/ftse-100\/constituents/,
//             timeout: 10000,
//           }),
...
//       } catch (navError) {
//         console.warn(
//           `Attempt ${attempt} navigation fallback failed:`,
//           navError
//         );
//         lastError = navError;
//       }
...
test("Extract FTSE 100 top 10 constituents by % change", async ({ page }) => {
  ...
  const top10Constituents = await page.evaluate(() => {
    ...
    // Map rows to objects with name, price, and change
    const data = rows.map((row) => {
      ...
      return {
        ...
        changeValue: parseFloat(cells[3]?.innerText.replace("%", "")) || 0,
      };
    });
    ...
    return data
      ...
      .slice(0, 10)
      .map(({ name, price, change }) => ({ name, price, change }));
  });

  console.log("Top 10 Constituents by % Change:", top10Constituents);
});